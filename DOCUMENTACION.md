...
*It's Fun o'Clock* es una landing page de venta de relojes de pared 3D personalizados, impresos en PLA+ y fabricados a mano en Barcelona. El objetivo del proyecto web es doble:
...
├── piezaisometrica.html    — Vista auxiliar de pieza
│
├── css/
...
La fuente se autoaloja desde `fonts/MartianMono-NrxBd.woff2` mediante `@font-face` en `typography.css`. Se eligió la versión **estática** (no variable) para garantizar un renderizado correcto del delineado (`-webkit-text-stroke`) en todos los navegadores. Inter (cuerpo de texto) se sigue cargando desde Google Fonts.
...
El proyecto es estático (HTML + CSS + JS sin servidor necesario para el home). Para el configurador, el archivo GLB se carga mediante `GLTFLoader` que usa `fetch` internamente, por lo que necesita un servidor local para evitar errores de CORS:

```bash
# Con Python (viene preinstalado en macOS)
cd itsfunoclock
python3 -m http.server 8000
# Abrir http://localhost:8000
```

O con la extensión **Live Server** de VS Code (clic derecho → Open with Live Server).
